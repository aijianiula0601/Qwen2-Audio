...
# Qwen2-Audio Single Node Pretraining Script
# This script runs the pretraining stage of Qwen2-Audio on a single node
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Example usage:"
            echo "  bash $0 --model qwen2_0.5b --gpus-per-node 8"
            exit 0
            ;;
        *)
...
echo "=== Qwen2-Audio Single Node Pretraining ==="
...
# Build training command for single node
...
# Create log file
LOG_FILE="${OUTPUT_DIR}/pretrain_single.log"
...
if [ $? -eq 0 ]; then
    ...
    echo "  bash scripts/train_sft_single.sh --model $MODEL_TYPE --resume $OUTPUT_DIR/pytorch_model.bin"
else
